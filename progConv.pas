...
var
 ...
 //--------------------------------------------------------------------------------------------------------------------------------	  
// Saisie
// fonction de saisie du nombre √† afficher, r√©p√®te la saisie jusqu'√† ce que l'utilisateur donne un nombre compris entre 0 et 1 milliard
...
// IN : tableau d'entier
// OUT : tableau d'entiers initialisÇ
...
begin
 ...
 //--------------------------------------------------------------------------------------------------------------------------------	  
//Decoupage
// permet de placer dans un tableau les chiffres (jusqu'aux centi√®mes) du nombre en entr√©e arrondi au centi√®me
// IN : nombre = le nombre √† d√©couper
...
begin
	initab(ent);
	nbentier := round(nombre*100);    // on multiplie nombre par 100 pour faire passer les centi√®mes dans la partie enti√®re et on arrondi √† l'nbentier le plus proche (permet de r√®gler les approximations dues au type r√©el)
	i := 10;
	repeat
 ...
 //--------------------------------------------------------------------------------------------------------------------------------	  
 //init_let
// permet d'initailiser le tableau qui va recevoir le nombre en chaine de caractäre
// IN : tableau de chaines
// OUT : tableau de chaines initialisÇ
 ...
 //--------------------------------------------------------------------------------------------------------------------------------
 //reference_unitÇs
// permet de remplir la table de refÇrence pour les unitÇs
// IN : tableau vierge
// OUT : tableau rempli
 ...
 //-------------------------------------------------------------------------------------------------------------------------------- 
 //reference_dizaines
// permet de remplir la table de refÇrence pour les dizaines
// IN : tableau vierge
// OUT : tableau rempli
 ...
 //--------------------------------------------------------------------------------------------------------------------------------	
 //Centaine
// permet de determiner quelle chaine de caractäre associer au chiffre des centaines
// IN : index dans le tableau du nombre en chiffre, tableau du nombre en chiffre, tableau du nombre en chaine, tableau de rÇfÇrence 
// OUT : tableau du nombre en chaine complÇtÇ
 ...
 //--------------------------------------------------------------------------------------------------------------------------------	
 //Dizaines
// permet de determiner quelle chaine de caractäre associer au chiffre des dizaines
// IN : index dans le tableau du nombre en chiffre, tableau du nombre en chiffre, tableau du nombre en chaine, tableau de rÇfÇrence 
// OUT : tableau du nombre en chaine complÇtÇ
 ...
 //--------------------------------------------------------------------------------------------------------------------------------	
//UnitÇ
// permet de determiner quelle chaine de caractäre associer au chiffre des unitÇs
// IN : index dans le tableau du nombre en chiffre, tableau du nombre en chiffre, tableau du nombre en chaine, tableau de rÇfÇrence 
// OUT : tableau du nombre en chaine complÇtÇ
  ...
  //--------------------------------------------------------------------------------------------------------------------------------	
 //f_letrification
// permet de determiner quelle fonction utiliser pour remplir le tableau en chaines de caractäre en fonction de la position du chiffre dans le nombre
// IN : i tableau du nombre en chiffre, tableau du nombre en chaine, tableaux de rÇfÇrence 
// OUT : tableau du nombre en chaine complÇtÇ 	
 ...
 procedure f_lettrification (var let : array of string ; var ent : array of integer; var refu : array of string; var  refd : array of string); 	 	
    ...
    begin 	
        ...
        //*****************cas des unit√©s	 (hors de la virgule)
            i := 2;
            while i <= 8 do
            begin
                unite(i, let, ent, refu);
                i := i + 3;                                                                                             // on saute de 3 pour accrocher que les unit√©s de chaque triplet
            end;
        //*****************cas des centimes     
 ...
 //--------------------------------------------------------------------------------------------------------------------------------	  
 //Lettrification
// permet de remplir le tableau du nombre en chaine de caractäre avec une version prÇ-assemblÇe
...
begin
 ...
 //--------------------------------------------------------------------------------------------------------------------------------	  
// GestionMillions
// Assemble la cha√Æne correspondant aux millions, en respectant les r√®gles de fran√ßais
// IN : let = le tableau contenant les cha√Ænes qui correspondent aux chiffres du nombre saisi pas l'utilisateur
// OUT :  millions = cha√Æne qui correspond aux millions
...
begin 
 ...
 //--------------------------------------------------------------------------------------------------------------------------------	  
// GestionMilliers
// Assemble la cha√Æne correspondant aux milliers, en respectant les r√®gles de fran√ßais
// IN : let = le tableau contenant les cha√Ænes qui correspondent aux chiffres du nombre saisi pas l'utilisateur
// OUT :  milliers = cha√Æne qui correspond aux milliers
...
begin 
 ...
 //--------------------------------------------------------------------------------------------------------------------------------	  
// GestionUnites
// Assemble la cha√Æne correspondant aux unites, en respectant les r√®gles de fran√ßais
// IN : let = le tableau contenant les cha√Ænes qui correspondent aux chiffres du nombre saisi pas l'utilisateur
// OUT :  unites = cha√Æne qui correspond aux unit√©s
...
begin 
 ...
 //--------------------------------------------------------------------------------------------------------------------------------	  
// Assemblage
// Assemble les cha√Ænes du tableau pour obtenir les cha√Ænes √† afficher
// IN : let = le tableau contenant les cha√Ænes qui correspondent aux chiffres du nombre saisi pas l'utilisateur
// OUT :  millions = cha√Æne qui correspond aux millions
 // milliers = cha√Æne qui correspond aux milliers
 // unites = cha√Æne qui correspond aux unites (3 premiers chiffres avant la virgule)
 // euros = prend les valeurs 'euros', 'euro' ou 'zero euro' selon les cas
// centimes = cha√Æne qui correspond aux centimes
...
begin
 ...
 //--------------------------------------------------------------------------------------------------------------------------------	  
 // Conversion
 // procedure principale, permet de convertir le nombre entrÇ en chiffres en version en toutes lettres
// IN : tableau du nombre en chiffre
// OUT : nombre en chaine de caractäre
